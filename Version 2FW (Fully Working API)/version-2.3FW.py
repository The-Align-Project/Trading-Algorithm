...
class AlpacaTradingAlgorithm:
    ...
    def round_to_valid_price(self, price):
        """Round price to valid increment for Alpaca"""
        if price >= 1.0:
            # Stocks over $1 must be in penny increments
            return round(price, 2)
        else:
            # Sub-dollar stocks can be in sub-penny increments
    ...
    def place_bracket_order(self, symbol, qty, side, limit_price=None, stop_loss=None, take_profit=None):
        ...
        try:
            ...
            if limit_price:
                order_data['limit_price'] = self.round_to_valid_price(limit_price)
            
            if order_class == 'bracket':
                # Round stop loss and take profit to valid increments
                rounded_stop_loss = self.round_to_valid_price(stop_loss)
                rounded_take_profit = self.round_to_valid_price(take_profit)
                
                order_data['stop_loss'] = {'stop_price': rounded_stop_loss}
                order_data['take_profit'] = {'limit_price': rounded_take_profit}
                
                print(f"   Rounded SL: ${rounded_stop_loss:.2f}, TP: ${rounded_take_profit:.2f}")
            ...
            # Try placing a simple market order instead
            try:
                simple_order = self.api.submit_order(
                    symbol=symbol,
                    qty=qty,
                    side=side,
                    type='market',
                ...
                print(f"✅ Simple market order placed instead: {side} {qty} shares of {symbol}")
                return simple_order
            except Exception as simple_error:
                print(f"❌ Error placing simple order: {simple_error}")
                return None
            ...
            try:
                ...
                for symbol in symbols:
                    ...
                    if signal == 'BUY' and not current_position:
                        ...
                        if qty > 0:
                            stop_loss = self.round_to_valid_price(current_price - (atr * 2))
                            take_profit = self.round_to_valid_price(current_price + (atr * 3))
    ...
    def print_summary(self):
        ...
        if self.alpaca_connected:
            ...
            print(f"💰 Current Capital: ${self.current_capital:.2f}")
            print(f"📈 Portfolio Value: ${self.get_simulation_portfolio_value():.2f}")
            total_return = ((self.get_simulation_portfolio_value() - self.initial_capital) / self.initial_capital) * 100
