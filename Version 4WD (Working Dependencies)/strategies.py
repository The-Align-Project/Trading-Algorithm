...
class AdvancedTradingStrategies:
    ...
    def momentum_breakout_strategy(self, data: pd.DataFrame) -> TradeSignal:
        ...
        current = data.iloc[-1]
        symbol = data.attrs.get('symbol', getattr(data, 'symbol', 'UNKNOWN'))
        ...
        # Conservative thresholds: require stronger signals
        if buy_score >= 2:  # TESTING: Lowered threshold
        ...
        elif sell_score >= 1:  # TESTING: Lowered threshold
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = None) -> List[TradeSignal]:
        """Generate signals from all strategies and return the best ones"""
        # Get symbol from parameter or data attributes
        if symbol is None:
            symbol = data.attrs.get('symbol', getattr(data, 'symbol', 'UNKNOWN'))
        
        # Store symbol in data for individual strategy methods
