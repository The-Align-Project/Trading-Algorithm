...
class PortfolioManager:
    ...
    def add_position(self, symbol: str, quantity: int, entry_price: float, 
                    ...
                    strategy: str = "") -> bool:
        """Add a new position to the portfolio or update existing position"""
        try:
            ...
            if symbol in self.positions:
                ...
                # Update cash for the new shares only
                cost = abs(quantity) * entry_price
                self.cash -= cost
                
                self.logger.info(f"Updated position: {symbol} - Added {quantity} shares at ${entry_price:.2f} (New total: {total_quantity} shares, Avg: ${new_avg_price:.2f})")
                return True
            else:
                # Create new position
                position = Position(
                ...
                self.logger.info(f"Added new position: {quantity} shares of {symbol} at ${entry_price:.2f}")
                return True
    ...
    def export_performance_report(self, filename: str = None) -> str:
        ...
        if not filename:
            ...
            filename = f"results/live/performance_report_{timestamp}.txt"
